...
/// - Read the first stdout line, which the backend prints as `OMNIBOT_BACKEND_PORT=<port>`,
...
final class BackendSupervisor {
    ...
    private func _startLocked() throws {
        ...
        // Resolve data dir: ~/Library/Application Support/OmnibotApp/
        ...
        // Logs dir: ~/Library/Logs/OmnibotApp/
        let logsDir = (NSHomeDirectory() as NSString).appendingPathComponent("Library/Logs/OmnibotApp")
        ...
        proc.arguments = ["--data-dir", dataDir, "--bind", "127.0.0.1:0"]
        let stdoutPipe = Pipe()
        let stderrPipe = Pipe()
        ...
        dbg("spawned backend pid=\(proc.processIdentifier), waiting for port file...")

        let port = try waitForPortFile(at: portFilePath, timeoutSeconds: 8)
        self.port = port
        setenv("OMNIBOT_BACKEND_PORT", String(port), 1)  // best-effort
    ...
    private func readPort(from handle: FileHandle) throws -> UInt16 {
        // Read until a newline, timeout 8 s.
        ...
        while Date() < deadline {
            ...
            if let newlineRange = buffer.firstRange(of: Data([0x0A])) {
                ...
                throw NSError(domain: "BackendSupervisor", code: 3,
                              userInfo: [NSLocalizedDescriptionKey: "unexpected stdout: \(String(data: line, encoding: .utf8) ?? "")"])
            }
        }
