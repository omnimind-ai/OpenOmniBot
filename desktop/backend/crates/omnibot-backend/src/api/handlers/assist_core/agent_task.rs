...
use omnibot_agent::{AgentCallback, AgentOrchestrator, AgentRunInput, AgentStreamEvent};
...
fn resolve_profile(
...
) -> AppResult<ModelProviderProfile> {
    let id = args
        .get("modelProfileId")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string());
    ...
    if let Some(id) = id {
        ...
        cfg.profiles.into_iter().next().ok_or_else(|| {
            AppError::InvalidArgument(
                "no model profile configured; set one in settings first".into(),
            )
        })
    }
}
...
async fn spawn_task(
    ...
    if let Some(cid) = conversation_id {
        ...
        .await?;
        let _ = ConversationRepo::touch(
            session.state.db.pool(),
            cid,
            Some(&truncate(&query, 120)),
            1,
        )
        .await;
    }
    ...
    session.state.active_tasks.insert(
        task_id.clone(),
        ActiveTaskHandle {
            cancel: cancel.clone(),
    ...
    tokio::spawn(async move {
        ...
        let result = if enable_tools {
            ...
            match session_for_task
                .state
                .llm_client
                .stream_turn(request, &endpoint, chat_sink)
                .await
            {
                Ok(t) => {
                    ...
                    omnibot_agent::AgentResult::Finished {
                        rounds: 1,
                ...
                Err(e) => {
                    ...
                    cb.on_stream_event(AgentStreamEvent::error(&task_id, &mode, 0, e.to_string()))
                        .await;
                    omnibot_agent::AgentResult::Error {
                        message: e.to_string(),
                    }
                }
            }
