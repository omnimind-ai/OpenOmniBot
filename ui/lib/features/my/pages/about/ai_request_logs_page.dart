...
class _AiRequestLogsPageState extends State<AiRequestLogsPage> {
  ...
  Future<void> _copyJson(String label, String content) async {
    ...
    if (!mounted) return;
    showToast(
      AppTextLocalizer.text('$label已复制'),
        ...
        Row(
          children: [
            ...
            Expanded(
              child: _buildOverviewMetric(
                context,
                label: AppTextLocalizer.text('成功'),
            ...
            Expanded(
              child: _buildOverviewMetric(
                context,
                label: AppTextLocalizer.text('失败'),
          ...
          Row(
            children: [
              ...
              TextButton(
                ...
                child: Text(AppTextLocalizer.text('复制')),
              ),
            ],
          ),
  ...
  Widget _buildEmptyState(BuildContext context) {
    ...
    return Padding(
      ...
      child: Text(
        AppTextLocalizer.text('最近还没有 AI 请求日志'),
        style: TextStyle(
          fontFamily: AppTextStyles.fontFamily,
          fontSize: 14,
          height: 1.5,
  ...
  Widget _buildErrorState(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            AppTextLocalizer.text('加载请求日志失败'),
            style: TextStyle(
              fontFamily: AppTextStyles.fontFamily,
              fontSize: 16,
              fontWeight: FontWeight.w600,
                    ...
                    ? Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildInfoRow(
                              context,
                              AppTextLocalizer.text('请求地址'),
                              log.url,
                            ),
                            _buildInfoRow(
                              context,
                              AppTextLocalizer.text('请求方法'),
                              log.method,
                            ),
                            if (log.errorMessage.trim().isNotEmpty)
                              _buildInfoRow(
                                context,
                                AppTextLocalizer.text('错误信息'),
                                log.errorMessage,
                              ),
                            const SizedBox(height: 4),
                            const SettingsSectionTitle(
                            ...
                            _buildJsonBlock(
                              context: context,
                              title: AppTextLocalizer.text('请求 JSON'),
                              content: log.requestJson,
                            ),
                            const SizedBox(height: 12),
                            _buildJsonBlock(
                              context: context,
                              title: AppTextLocalizer.text('响应 JSON'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: CommonAppBar(
        title: AppTextLocalizer.text('请求日志'),
        primary: true,
        actions: [
          IconButton(
            onPressed: _loadLogs,
