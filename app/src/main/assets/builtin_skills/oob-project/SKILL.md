---
name: oob-project
description: OOB Workbench Project 完整生命周期总入口：新建 Project、更新、蒸馏、审查、结构化数据、Project Tool/API、Agent action、轻量 Display、热更新、导出、PROJECT_SOUL/PROJECT_CONTEXT 维护。Use when the user says "帮我做一个", "我想创建一个", "新建 Project", "build me a", "make a [X] tracker/tool/app", "改一下界面", "加个字段", "新增功能", "修复", "project review", "审查 project", "总结成我们的 project", "蒸馏能力", "提取核心设计", "生成 soul", "把这个 app/项目变成 OOB Project", "产品分析", "GitHub 仓库分析", or any intent to create, distill, modify, review, export, or maintain a persistent personal workflow or tool.
---

# OOB Project

OOB Project 是 **OOB 内置核心能力**，不是独立插件边界。Project 的创建、更新、蒸馏、结构化数据、Tool/API、Agent action、Display、审查、导出和项目文档维护都归本 skill。
...
| 设计或修 Display/HTML | `references/frontend-guide.md`；需要样例时读 `references/html-patterns.md` |
...
- Project 的核心不是“生成一个网页”，而是把用户的一个反复发生的小工作流沉淀成可持续使用的数据、工具和默认可见入口。HTML 只是当前默认 Display 适配层。
...
字段名是 Entity Schema、Project Tool、Display 的共同语言。一处字段名不同，数据层、工具层、展示层就会断开。

```
entity schema  -> fields.kcal: number
api inputSchema -> kcal: "number"
toViewItem()   -> kcal: Number(f.kcal || 0)
Display/HTML  -> ${vi.kcal} 大卡
```
...
当前默认 Display renderer 是 `frontend/html/` 下的静态 HTML/CSS/JS。它默认负责：

- Agent 触发按钮。
- `project.items` 列表/仪表展示。
- 归档、删除、轻量编辑等已存在数据操作。

HTML 默认不负责多字段录入表单或复杂业务逻辑。用户明确要求“界面里直接编辑”时，才加轻量 inline 编辑。需要长期复用的能力必须落在 Project Tool、Agent action、script 或 Project 文档里。
