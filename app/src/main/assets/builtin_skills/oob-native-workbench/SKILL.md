...
6. Frontend viewport: expose the generated Display from the Home top Workbench button so the user can tap the Workbench mark beside the Chat / Workspace slider and immediately see the active Project's current frontend in an embedded child window. Keep Project switching, Display selection, delete/export, Workspace entry, and API stats in the simplified `/workbench/projects` manager; do not mix those controls into the generated frontend. The Project info popup should teach this boundary without adding a second creation flow.
