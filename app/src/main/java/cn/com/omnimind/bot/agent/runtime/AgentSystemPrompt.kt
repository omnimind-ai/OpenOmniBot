    ...
    fun build(
        ...
        val workbenchProjectOperationRules = when (locale) {
            PromptLocale.ZH_CN -> """
                ...
                - HTML UI 创建：提供 `htmlFiles`，至少含 `index.html`。移动端自适应布局用 `<meta name="viewport" content="width=device-width, initial-scale=1">`；宽屏报告/PPT 用 `<meta name="viewport" content="width=1280">`。
            """.trimIndent()
            PromptLocale.EN_US -> """
                ...
                - HTML UI creation: provide `htmlFiles` with at least `index.html`. Use `<meta name="viewport" content="width=device-width, initial-scale=1">` for mobile-first layouts; use `<meta name="viewport" content="width=1280">` for wide-screen reports or presentations.
            """.trimIndent()
        }
