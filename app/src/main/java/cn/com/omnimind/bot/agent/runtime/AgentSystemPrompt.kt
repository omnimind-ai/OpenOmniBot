    ...
    fun build(
        ...
        val workbenchProjectOperationRules = when (locale) {
            ...
            PromptLocale.EN_US -> """
                ...
                - When the user asks to `create project`, create a Project, or generate an OOB page with frontend/backend behavior, call `workbench_project_create`. Todo is only one demo; for normal vibe projects prefer `templateId=schema_app` and provide entityName, description, initialItems, or apis. After creation, call `workbench_project_activate` to make it the current workspace; call `workbench_project_open` when the user should see it. To seed business data, call `workbench_api_list` first and then `workbench_api_call`.
