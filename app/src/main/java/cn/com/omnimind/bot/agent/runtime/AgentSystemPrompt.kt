    ...
    fun build(
        ...
        val workbenchProjectOperationRules = when (locale) {
            PromptLocale.ZH_CN -> """
                ...
                - HTML UI 创建：提供 `htmlFiles`，至少含 `index.html`。默认目标是真机右侧 Workspace WebView 的手机竖屏，约 360-430dp 宽；首屏必须紧凑展示标题、当前状态/摘要、主操作或关键结论。移动交互 UI 和竖屏报告都用 `<meta name="viewport" content="width=device-width, initial-scale=1">`，单列布局，最大宽度约 430px，避免桌面 hero、大卡片堆叠、横向宽表格；竖屏报告用手机宽文章、摘要、分节、响应式图表、堆叠对比行。只有用户明确要宽屏报告/PPT/横向对比画布时才用 `<meta name="viewport" content="width=1280">`。
            """.trimIndent()
            PromptLocale.EN_US -> """
                ...
                - HTML UI creation: provide `htmlFiles` with at least `index.html`. The default target is the real-phone right-side Workspace WebView in portrait, roughly 360-430dp wide. The first viewport must be compact and useful: title, current state/summary, primary action, or key findings. Mobile interaction UI and portrait reports both use `<meta name="viewport" content="width=device-width, initial-scale=1">`, one-column layout, about 430px max width, and no desktop hero, oversized card stacks, or wide horizontal tables. Portrait reports should be phone-width articles with an executive summary, sections, responsive charts, and stacked comparison rows. Use `<meta name="viewport" content="width=1280">` only when the user explicitly asks for a wide report, slide deck, or landscape comparison canvas.
            """.trimIndent()
        }
