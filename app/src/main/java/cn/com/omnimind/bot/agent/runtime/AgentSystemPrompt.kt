    ...
    fun build(
        ...
        val workbenchProjectOperationRules = when (locale) {
            PromptLocale.ZH_CN -> """
                ...
                - HTML 数据规则：`project.items` 是真实持久化数据，不是 mock，不要在 HTML 里 hardcode 数据数组。页面加载时必须调 `const project = await window.oob.getProject(); renderItems(project.items);` 读取真实数据。同步 CRUD 操作后从 `result.project.items` 刷新 UI；异步 agent_task 操作后在 `window.oob.onProjectUpdated(callback)` 里刷新 UI。
            """.trimIndent()
            PromptLocale.EN_US -> """
                ...
                - HTML data rules: `project.items` is real persistent data, not mock. Never hardcode data arrays in HTML. On page load call `const project = await window.oob.getProject(); renderItems(project.items);`. After sync CRUD, re-render from `result.project.items`. After async agent_task, re-render inside `window.oob.onProjectUpdated(callback)`.
            """.trimIndent()
        }
