    ...
    fun build(
        ...
        val workbenchProjectOperationRules = when (locale) {
            PromptLocale.ZH_CN -> """
                ...
                - 创建 Project 是审慎控制面决策。只有用户明确说“创建/新建 Project/帮我做一个工作台/做一个可显示的 OOB Project”时，才允许调用 `workbench_project_create`。Todo 只是一种 demo；用户明确要“随手记/Inbox/收文本链接摘要”的 Project 时用 `templateId=quick_capture_inbox`；普通 vibe project 优先用 `templateId=schema_app`，写清 entityName、description、initialItems 或 apis。创建后调用 `workbench_project_activate` 设为当前工作环境；如需展示，再调用 `workbench_project_open`。初始化业务数据时先 `workbench_api_list`，再用 `workbench_api_call` 调 Project 业务 API。
                - 生成/展示规范：Project Display 是用户正在使用的应用首屏，不是 Project 摘要卡。创建或热更新 Project 时，首屏只放业务工作流、输入、列表、筛选、状态和业务按钮；不要在 Display 里展示 Project id、template、Toolbox、API 数量、executor、schema、Workspace 路径、data/log 路径、`OOB native UI`、`backend/api_spec.json` 或“这是生成前端”之类的实现说明。这些信息只能放在 `/workbench/projects` 详情、info 弹窗、MCP resources、日志或 debug 文档里。
                ...
                - 用户只是说“记一下/总结这个链接/归档这条/给当前 todo project 增加一条 todo”这类业务操作时，不要创建 Project，也不要改数据文件；先确认 active Project（必要时 `workbench_project_active_get`），再 `workbench_api_list(projectId)`，然后调用 `workbench_api_call(projectId, apiId, inputs)`。如果没有 active Project，要求用户先明确创建或选择 Project。
                ...
                - Flutter UI 热更新流程：当 `workbench_project_hot_update` 返回 `requiresAgentRegeneration=true`，说明 Project 有 Flutter 源码需要 AI 重写。必须立即：(1) 从返回结果 project.frontendFlutter.sources 的 entryFile（默认 lib/main.dart）读取现有 Dart 代码；(2) 结合 instructions 字段和 frontendContext 用户注释，生成完整的可直接编译的新 Dart 文件，不能写改动行、省略号或注释占位；(3) 调用 workbench_project_update 传 flutterFiles。重写必须保持：OobProjectWidget(dynamic _) 入口类、所有 _channel.invokeMapMethod('workbenchApiCall',...) 调用的 apiId 不变、只用 flutter/material.dart 和 flutter/services.dart、颜色用 Colors.xxx。
            """.trimIndent()
            PromptLocale.EN_US -> """
                ...
                - Creating a Project is a deliberate control-plane decision. Only call `workbench_project_create` when the user explicitly asks to create/new a Project, build a workbench, or make a visible OOB Project. Todo is only one demo; when the user explicitly asks for a quick-capture/inbox Project for text, links, summaries, or later-read items, use `templateId=quick_capture_inbox`; for normal vibe projects prefer `templateId=schema_app` and provide entityName, description, initialItems, or apis. After creation, call `workbench_project_activate` to make it the current workspace; call `workbench_project_open` when the user should see it. To seed business data, call `workbench_api_list` first and then `workbench_api_call`.
                - Generation/display rule: a Project Display is the app surface the user is using, not a Project summary card. When creating or hot-updating a Project, the first viewport should contain only the business workflow, inputs, lists, filters, status, and business actions. Do not show Project id, template, Toolbox, API counts, executor kind, schema, Workspace path, data/log paths, `OOB native UI`, `backend/api_spec.json`, or "this is generated" implementation copy inside the Display. Those details belong only in `/workbench/projects` detail, the info popup, MCP resources, logs, or debug documentation.
                ...
                - Flutter UI hot-update flow: when `workbench_project_hot_update` returns requiresAgentRegeneration=true, the Project has Flutter source files that need AI rewriting. You must immediately: (1) read the existing Dart from the returned project.frontendFlutter.sources at the entryFile (default lib/main.dart); (2) use the instructions field and frontendContext annotations to generate a complete, directly compilable new Dart file — never write a diff, ellipsis, or placeholder comment; (3) call workbench_project_update with flutterFiles. Preserve: OobProjectWidget(dynamic _) entry class, all _channel.invokeMapMethod('workbenchApiCall',...) calls with their existing apiIds, only package flutter/material.dart and flutter/services.dart imports, colors as Colors.xxx Material constants.
            """.trimIndent()
        }
