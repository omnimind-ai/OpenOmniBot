    ...
    fun build(
        ...
        val workbenchProjectOperationRules = when (locale) {
            PromptLocale.ZH_CN -> """
                ...
                - 创建 Project 是审慎控制面决策。只有用户明确说“创建/新建 Project/帮我做一个工作台/做一个可显示的 OOB Project”时，才允许调用 `workbench_project_create`。OOB Workbench 是 AI 产品输出展示层，不是一次性应用生成器，也不是预置应用目录。创建时只提供 `projectId`、`apis`、`initialItems`、`htmlFiles`（或 displays）。**任何需要 UI 展示的场景都优先提供 `htmlFiles`，至少包含 `index.html`**，让右侧 Flutter Host 走 `html_webview`；结构化数据和操作通过 Project Tools + 默认 Project Display 承载。不要提供 `flutterFiles`，AI 生成的 Dart 代码在 flutter_eval 运行时会因 API 子集限制编译失败。创建后调用 `workbench_project_activate` 设为当前工作环境；如需展示，再调用 `workbench_project_open`。初始化业务数据时先 `workbench_api_list`，再用 `workbench_api_call` 调 Project 业务 API。
                ...
                - HTML 数据规则：`project.items` 是真实持久化数据，不是 mock，不要在 HTML 里 hardcode 数据数组。每个 item 是 Workbench envelope：`{ id, title, status, fields }`；业务字段只在 `item.fields` 下，不能把 `amount/type/category/date/note` 等当作顶层 `item.amount` 读取。页面加载时必须调 `const project = await window.oob.getProject();`，再把 `(project.items || [])` 先过滤 active、再从 `item.fields` 归一化成 UI view model 后渲染。
                - HTML 工具返回规则：`window.oob.callApi(apiId, inputs)` 返回 envelope，不是 raw list。读取 `result.success` 判断成功，读取 `result.outputs` 取 API 输出，读取 `result.project` 取更新后的 Project，错误展示 `result.errorCode/result.errorMessage`。列表 API 用 `result.outputs.items || result.project.items`，不要读 `result.items`；同步 CRUD 后从 `result.project.items` 刷新，缺失时重新 `getProject()`；异步 agent_task 后只在 `window.oob.onProjectUpdated(callback)` 里刷新。
                - HTML 后端契约规则：如果 Project API 使用 `script` 或 `workspace_python_script`，必须同时提供真实存在的 `backend/scripts/...` 脚本；否则会返回 `SCRIPT_NOT_FOUND`。简单统计优先可直接从 `project.items[].fields` 在 HTML 内计算，不要注册一个缺文件的统计 API。
            """.trimIndent()
            PromptLocale.EN_US -> """
                ...
                - Creating a Project is a deliberate control-plane decision. Only call `workbench_project_create` when the user explicitly asks to create/new a Project, build a workbench, or make a visible OOB Project. OOB Workbench is an AI-product output display layer, not a one-off app generator and not a preset app catalog. Provide only `projectId`, `apis`, `initialItems`, and `htmlFiles` (or displays). **Always provide `htmlFiles` with at least `index.html` for any UI display need** so the Flutter Host uses the `html_webview` renderer. Structured data and actions use Project Tools plus the default Project Display. Do not provide `flutterFiles` — AI-generated Dart code routinely fails to compile in the `flutter_eval` runtime due to unsupported API subsets. After creation, call `workbench_project_activate` to make it the current workspace; call `workbench_project_open` when the user should see it. To seed business data, call `workbench_api_list` first and then `workbench_api_call`.
                ...
                - HTML data rules: `project.items` is real persistent data, not mock. Never hardcode data arrays in HTML. Each item is a Workbench envelope: `{ id, title, status, fields }`; business fields live only under `item.fields`, so do not read `amount/type/category/date/note` as top-level `item.amount`. On page load call `const project = await window.oob.getProject();`, then filter active items and normalize from `item.fields` into UI view models before rendering.
                - HTML tool result rules: `window.oob.callApi(apiId, inputs)` returns an envelope, not a raw list. Read `result.success`, API outputs from `result.outputs`, updated Project state from `result.project`, and errors from `result.errorCode/result.errorMessage`. List APIs use `result.outputs.items || result.project.items`, never `result.items`; after sync CRUD re-render from `result.project.items`, falling back to `getProject()` when absent; after async agent_task re-render only inside `window.oob.onProjectUpdated(callback)`.
                - HTML backend contract rules: if a Project API uses `script` or `workspace_python_script`, the referenced `backend/scripts/...` file must exist or the API will return `SCRIPT_NOT_FOUND`. Prefer computing simple stats in HTML from `project.items[].fields` over registering a missing stats API.
            """.trimIndent()
        }
